...
try {
    ...
    if (empty($_POST) || empty($_POST['id_identity_step']) || empty($_POST['position_identity_step_dir'])) {
        throw new Exception("Error : Args not valid", 1);
    }

    $Identity = new Identity($User);
    $Identity->_moveIdentityStep(App::encrypt_decrypt('decrypt', $_POST['id_identity_step']), $_POST['position_identity_step_dir']);
